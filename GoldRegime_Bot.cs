// ═══════════════════════════════════════════════════════════════
// GoldRegime 115.5% ret / 5% DD
// Category: HIGH | Generated: 2026-03-18
// Gold Regime Lab v3 — XAUUSD 3-State Regime cBot
// ═══════════════════════════════════════════════════════════════
// Backtest Stats: Return 115.5% | WR 60% | PF 4.77 | MaxDD 5%
// Trades: 10 (6W/4L) | AvgR 2
// ═══════════════════════════════════════════════════════════════
//
// ╔═══════════════════════════════════════════════════════════╗
// ║  RISK WARNINGS — 1 parameter(s) outside safe definitions  ║
// ║  Computed Risk Rating: MED      RR=4:1 Risk=0.5% Lev=10x  ║
// ╚═══════════════════════════════════════════════════════════╝
// ○ INFO    : Expansion Threshold: current 1.2x, safe range 1.02–1.15x (4% above safe maximum (1.15x))
...
namespace cAlgo.Robots
{
    ...
    public class Goldregime1155Ret5Dd : Robot
    {
        // ── Core Parameters ──────────────────────────────────────────
        ...
        [Parameter("ATR Stop Multiplier", Group = "Entry", DefaultValue = 3, MinValue = 0.5, MaxValue = 5.0, Step = 0.1)]
        ...
        // ── Regime Detection ─────────────────────────────────────────
        [Parameter("Compression Threshold", Group = "Regime", DefaultValue = 0.018, MinValue = 0.001, MaxValue = 0.1, Step = 0.001)]
        public double CompressionThreshold { get; set; }

        [Parameter("Expansion Threshold", Group = "Regime", DefaultValue = 1.2, MinValue = 1.01, MaxValue = 3.0, Step = 0.01)]
        public double ExpansionThreshold { get; set; }

        [Parameter("Range Width Bars (H4)", Group = "Regime", DefaultValue = 10, MinValue = 5, MaxValue = 50)]
        public int RangeWidthBars { get; set; }

        [Parameter("Midpoint Band %", Group = "Regime", DefaultValue = 0.08, MinValue = 0.01, MaxValue = 0.5, Step = 0.01)]
        public double MidpointBandPct { get; set; }

        // ── Entry Filters ────────────────────────────────────────────
        [Parameter("Retest Buffer", Group = "Entry", DefaultValue = 10, MinValue = 0.5, MaxValue = 50.0, Step = 0.5)]
        public double RetestBuffer { get; set; }

        [Parameter("Wick Ratio", Group = "Entry", DefaultValue = 0.6, MinValue = 0.3, MaxValue = 5.0, Step = 0.1)]
        ...
        [Parameter("Entry Window Bars", Group = "Session", DefaultValue = 2, MinValue = 0, MaxValue = 12)]
        ...
        // ── Risk Management ──────────────────────────────────────────
        [Parameter("Risk Per Trade %", Group = "Risk", DefaultValue = 0.5, MinValue = 0.1, MaxValue = 10.0, Step = 0.05)]
        ...
        [Parameter("Post-Loss Cooldown Bars", Group = "Risk", DefaultValue = 4, MinValue = 0, MaxValue = 20)]
        ...
        [Parameter("ATR Risk Scale Threshold", Group = "Risk", DefaultValue = 1.15, MinValue = 1.01, MaxValue = 5.0, Step = 0.01)]
        public double AtrRiskScaleThreshold { get; set; }

        [Parameter("ATR Risk Scale Factor", Group = "Risk", DefaultValue = 0.5, MinValue = 0.1, MaxValue = 1.0, Step = 0.05)]
        ...
        {
            _botLabel = "Goldregime1155Ret5Dd_" + Server.Time.Ticks;
            ...
            Print("Goldregime1155Ret5Dd started — HIGH risk category");
            Print("Config: RR=4 ATR=3 Risk=0.5% Lev=10x");
        }
        ...
        {
            Print("Goldregime1155Ret5Dd stopped. Final balance: " + Account.Balance);
        }
    }
}
